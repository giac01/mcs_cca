...
if (!("ccatools" %in% installed.packages())) {
  devtools::install_github("giac01/ccatools@af914be1b99151a1b309150f8d2d5141c6ca83f5") #The @ specifies a specific commit (version) of the package on github
}
...
PlotCorrelationMatrix(cca_splithalf_noprocrustes$model_results$variates, includeN = FALSE, na.color = "white")
```
...
Variates_Correlation_Test = 
  Variates_Plot %>% 
    ...
    PlotCorrelationMatrix(., includeN = FALSE)
...
Plot1=
ggplot(data=Variates_Plot, aes(x=`ERF 1`, y=`Outcome 1`)) + 
  geom_point(alpha=.4, shape=16) + 
  jtools::theme_apa() + coord_fixed() +
  labs(x="1st CCA Environmental Risk Factor Latent Variable",
       y="1st CCA Outcome Latent Variable") 

Plot2=
ggplot(data=Variates_Plot, aes(x=`ERF 2`, y=`Outcome 2`)) + 
  geom_point(alpha=.4, shape=16) + 
  jtools::theme_apa() + coord_fixed() +
  labs(x="2nd CCA Environmental Risk Factor Latent Variable",
       y="2nd CCA Outcome Latent Variable") 

Plot3=
ggplot(data=Variates_Plot, aes(x=`ERF 3`, y=`Outcome 3`)) + 
  geom_point(alpha=.4, shape=16) + 
  jtools::theme_apa() + coord_fixed() +
  labs(x="3rd CCA Environmental Risk Factor Latent Variable",
       y="3rd CCA Outcome Latent Variable") 
...
PlotCorrelationMatrix(Variates_Plot, includeN = FALSE, zerotext = "0", aligntext = FALSE)
...
plot_data = df0_imputed_BothGroups[c(envvar[envvar_Order],outcomes[Outcome_Order])  ]
  colnames(plot_data) = c(envvar_labels[envvar_Order],outcomes_labels[Outcome_Order])
gp = PlotCorrelationMatrix(plot_data, includeN = FALSE, textadjust = 1.3)
gp
...
plot_data = df0_imputed_Teach_BothGroups[c(envvar[envvar_Order],Outcome_Ordered2)  ]
...
plot_data = df0_imputed_BothGroups[c(envvar[envvar_Order],outcomes[Outcome_Order])  ]
  # cbind.data.frame(c(envvar_labels[envvar_Order],outcomes_labels[Outcome_Order]),colnames(plot_data))
  colnames(plot_data) = c(envvar_labels[envvar_Order],outcomes_labels[Outcome_Order])
gp2 = PlotCorrelationMatrix_reduced(plot_data, textadjust = 1.3)
gp2

#Export File 
ggsave(file.path("Plots","CorMat_BothGroups_NONUM.pdf"), plot=gp2, device="pdf", width=5, height=5)
rm(gp2,plot_data)
```
