...
EthnicityTbl =
  Ethnicity %>% 
    ...
    data.frame() %>%
    dplyr::arrange(Freq) %>%
    dplyr::mutate(Percent = format(Freq/sum(Freq), nsmall=4,digits=0))
...
```{r}
S1Geo = fread("Data/MCS Data/Raw Data/UKDA-4683-tab MCS1/tab/mcs1_geographically_linked_data.tab", stringsAsFactors = FALSE, data.table = FALSE)
...
S1Geo %>%
  ...
  summarise(N=n()) %>% 
  mutate(Perc = round(N/sum(N), digits=4))
...
S1Der = fread("Data/MCS Data/Raw Data/UKDA-4683-tab MCS1/tab/mcs1_derived_variables.tab", stringsAsFactors = FALSE, data.table = FALSE)
...
# Proportion of productive families in S5 & S6 excluded from analyses
